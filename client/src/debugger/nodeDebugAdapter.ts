...
function fixNodeInternalsSkipFiles(args: CommonRequestArgs): void {
  if (args.skipFiles) {
...
export class NodeDebugAdapter extends ChromeDebugAdapter {
  ...
  get launchAttachArgs(): CommonRequestArgs {
  ...
  private extensionHostLaunch(
    ...
    const launchVSCodeArgs: LaunchVSCodeArguments = {
      args: args,
  ...
  public async attach(args: AttachRequestArguments): Promise<void> {
    try {
      if (typeof args.enableSourceMapCaching !== "boolean") {
  ...
  private supportsStepBack(): boolean {
    return this._domains.has("TimeTravel" as keyof Crdp.ProtocolApi);
  }
  ...
  private captureStderr(
    ...
    nodeProcess.stderr.on("data", (data: string) => {
      ...
      if (
        (this._handlingEarlyNodeMsgs || noDebugMode || this._captureFromStd) &&
        !this._launchAttachArgs._suppressConsoleOutput
      ) {
  ...
  private collectEnvFileArgs(args: LaunchRequestArguments): any {
  ...
  public async terminate(
    ...
    if (
      !this._attachMode &&
      !(this._launchAttachArgs as LaunchRequestArguments).useWSL &&
      this.nodeProcessId > 0
    ) {
  ...
  private async resolveProgramPath(
    ...
    if (this.jsDeterminant.isJavaScript(programPath)) {
      ...
      const generatedPath = await this.sourceMapTransformer.getGeneratedPathFromAuthoredPath(
        programPath
      );
  ...
  private getErrorResponseWithInfoLink(
    ...
    return Promise.reject(
      new ErrorWithMessage({
        ...
        showUser: true,
        url:
          "http://go.microsoft.com/fwlink/?linkID=534832#_" + infoId.toString(),
